...
class ResizeImages:
    def __init__(self, 
             ...
             out_path=None,
             multi_part=True,  
             resize_dim = [(64, 64), (64,64), (192, 256)],
             labels_from_files=True):
        ...
        if multi_part:
            assert len(resize_dim) == 3
            self.face_dim = resize_dim[0] 
            self.flank_dim = resize_dim[1]
            self.full_dim = resize_dim[2]
        else:
    ...
    def run(self):
        ...
        isExist = os.path.exists(self.out_path+'/full')                  
        # Create a new directory because it does not exist     
        if not isExist:
           os.makedirs(self.out_path+'/face/') 
           os.makedirs(self.out_path+'/flank/') 
           os.makedirs(self.out_path+'/full/') 
